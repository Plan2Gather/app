...
export function createMeshedGroupData(groupAvalabilities: [UserAvailability]) {
    let periodsByDay: { name: string; avalable: [number, number][] }[][] = [];
...
function combineTimeSlots(
    groupTimePeriods: { name: string; avalable: [number, number][] }[]
    ...
    for (let i = 0; i < slotSet.length - 1; i++) {
        ...
        for (const individualTimePeriods of groupTimePeriods) {
            if (
                isAvalable(
                    individualTimePeriods.avalable,
                    slotSet[i],
                    slotSet[i + 1]
                )
            ) {
...
function addToSortedSet(mySet: number[], value: number): void {
    ...
    if (mySet[indexToInsert] != value) {
        mySet.splice(
            indexToInsert !== -1 ? indexToInsert : mySet.length,
            0,
            value
        ); //ToDo: understand this. just stackOverflowed it and am a bit confused
    }
}

//ToDo: make this use binary search in stead of just iterating throught
function isAvalable(
    individualTimePeriods: [number, number][],
    start: number,
    end: number
): boolean {
